...
def _load_script_module() -> object:
    script_path = Path(__file__).resolve().parents[2] / "scripts" / "run_trial_stats_group.py"
    spec = spec_from_file_location("run_trial_stats_group", script_path)
...
def test_run_trial_stats_group_script_main_smoke(monkeypatch) -> None:
    case_dir = _make_case_dir("run_script_smoke")
    try:
