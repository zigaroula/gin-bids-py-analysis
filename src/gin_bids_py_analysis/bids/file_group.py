...
def build_subject_groups(
    ...
    *,
    ...
    ----------
    dataset:
        A :class:`~gin_bids_py_analysis.bids.BIDSDataset` already loaded.
    ieeg_filters:
        Entity filters forwarded to
        :meth:`~gin_bids_py_analysis.bids.BIDSDataset.get_files` to select
        the primary iEEG files (e.g. ``{"suffix": "ieeg", "extension": ".vhdr",
        "desc": "gammasm0"}``).
    secondary_filters:
        List of entity-filter dicts, each forwarded to
        :meth:`~gin_bids_py_analysis.bids.BIDSDataset.get_files` to discover
